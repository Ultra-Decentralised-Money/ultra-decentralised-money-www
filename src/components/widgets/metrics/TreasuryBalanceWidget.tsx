...
export const TreasuryBalanceWidget = () => {
  ...
  const getCurrentValue = () => {
    return data?.treasuryBalance && data.treasuryBalance.length > 0 
      ? data.treasuryBalance[data.treasuryBalance.length - 1].value 
      : 0;
  };

  const getChange = () => {
    if (!data?.treasuryBalance || data.treasuryBalance.length < 2) return 0;
    const current = data.treasuryBalance[data.treasuryBalance.length - 1].value;
    const previous = data.treasuryBalance[data.treasuryBalance.length - 2].value;
  ...
  return (
    <Widget
      ...
      description="Total ADA in the Cardano treasury available for funding"
      loading={loading}
      error={error}
